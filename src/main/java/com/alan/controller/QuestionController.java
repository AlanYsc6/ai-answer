...
public class QuestionController {
    ...
    private static final String GENERATE_QUESTION_SYSTEM_MESSAGE = "你是一位严谨的出题专家，我会给你如下信息：\n" +
            ...
            "2. 测评类应用严格按照下面的 json 格式输出题目、选项\n" +
            "```\n" +
            "[{\"options\":[{\"result\":\"I\",\"value\":\"选项内容\",\"key\":\"A\"},{\"result\":\"E\",\"value\":\"\",\"key\":\"B\"}],\"title\":\"题目标题\"}]\n" +
            "```\n" +
            "title 是题目，options 是选项，每个选项的 key 按照英文字母序（比如 A、B、C、D）以此类推，value 是选项内容，result是value对应的标记，用一位大写字母表示\n" +
            "3. 得分类应用严格按照下面的 json 格式输出题目、选项\n" +
            "```\n" +
            "[{\"options\":[{\"score\":0,\"value\":\"选项内容\",\"key\":\"A\"},{\"score\":1,\"value\":\"\",\"key\":\"B\"}],\"title\":\"题目标题\"}]\n" +
            "```\n" +
            "title 是题目，options 是选项，每个选项的 key 按照英文字母序（比如 A、B、C、D）以此类推，value 是选项内容，score是value对应的得分，用一位整数表示，正确答案为1，错误答案为0\n" +
            "4. 正确选项分布在不同位置，避免出现连续的题目正确选项的key相同的情况";
    ...
    public BaseResponse<List<QuestionContentDTO>> aiGenerateQuestion(
        ...
        // AI 生成
        String result = zhiPuAiUtil.doSyncRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage, null);
        // 截取需要的 JSON 信息
